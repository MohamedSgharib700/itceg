				...
				<div class="container">
					<ul class="nav nav-pills sort-source mt-xl mb-xl" data-sort-id="portfolio" data-option-key="filter" data-plugin-options="{'layoutMode': 'fitRows', 'filter': '*'}">
					   ...
					   @foreach($catalogue as $catalogue)
						...
						<li data-option-value=".{{$catalogue->catalogue}}" class=""><a href="#">{{$catalogue->catalogue}}</a></li>
						...
						<div class="sort-destination-loader sort-destination-loader-showing">
						 ...
						 @foreach($products as $products)
							 ...
							 @endphp
								...
								<li class="col-md-4 isotope-item m-none p-none {{$filter->catalogue}}">
								
									<div class="portfolio-item m-none">
										<a href="{{route('showProduct',['product'=>$products->custom_url])}}">
											<span class="thumb-info thumb-info-centered-info thumb-info-no-borders m-none">
												<span class="thumb-info-wrapper">
													<img style="height: 300px; width:500px;" src="{{asset('productsAdd/'.$products->image)}}" class="img-responsive" alt="">
													<span class="thumb-info-title">
														<span class="thumb-info-inner">{{$products->title}}</span>
														<span class="thumb-info-type">{{$products->sub_title}}</span>
													</span>
